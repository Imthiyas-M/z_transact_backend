...
# #         sync_cfg = db.query(EmailSyncSettings).filter_by(connection_id=conn_info.id).first()
# #
...
#
#
# def process_email_account(conn_id: str, save_dir: str, temp_dir: str, hash_path: str, email_pass: str = None):
#     db = SessionLocal()
#     try:
#         conn_info = db.query(EmailConnection).filter_by(id=conn_id).first()
#         if not conn_info:
#             raise ValueError("Email connection not found.")
#
#         sync_cfg = db.query(EmailSyncSettings).filter_by(connection_id=conn_info.id).first()
#         if not sync_cfg:
#             raise ValueError("Email sync configuration not found.")
#
#         # Use encrypted password if applicable
#         password = email_pass or conn_info.password
#         if not password:
...
def process_email_account(email_id: str, save_dir: str, temp_dir: str, hash_path: str, email_pass: str = None):
    ...
    try:
        conn_info = db.query(EmailConnection).filter_by(id=email_id).first()
        if not conn_info:
            raise ValueError("Email connection not found.")
        ...
        for folder in sync_cfg.email_folders:
            ...
            for eid in data[0].split():
                ...
                for part in attachments:
                    ...
                    final_pdf_path = os.path.join(save_dir, os.path.basename(temp_pdf_path))
